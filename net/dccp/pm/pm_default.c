...
static void pm_add_remote_addr(struct mpdccp_cb *mpcb, sa_family_t family, u8 id, union inet_addr *addr, u16 port)
{
	...
	if (family == AF_INET) {
		...
		mpdccp_pr_debug("Stored new remote IP %pI4:%u with id: %u", &addr->in, port, id);
	} else {
		remote_addr->addr.in6 = addr->in6;
		mpdccp_pr_debug("Stored new remote IP %pI6:%u with id: %u", &addr->in6, port, id);
	}
	
	list_add_tail_rcu(&remote_addr->address_list, &mpcb->premote_list);
	mpcb->cnt_remote_addrs += 1;
	return;
}
	...
	int				socklen)
{
	...
	pm_add_remote_addr(mpcb, remote_address->sa_family, 0, &rema, 0);
